...
async function moveMessages(uids, targetMailbox, sourceMailbox = DEFAULT_MAILBOX) {
  const imap = await connect();

  try {
    await openBox(imap, sourceMailbox, false);

    return new Promise((resolve, reject) => {
      imap.move(uids, targetMailbox, (err) => {
        if (err) reject(err);
        else resolve({ success: true, uids, action: `moved to ${targetMailbox}` });
      });
    });
  } finally {
    imap.end();
    ...
    switch (command) {
      ...
      case 'move':
        if (positional.length < 2) {
          throw new Error('Usage: node imap.js move <target-mailbox> <uid> [uid2...]');
        }
        const targetMailbox = positional[0];
        const uidsToMove = positional.slice(1);
        result = await moveMessages(uidsToMove, targetMailbox, options.mailbox);
        break;
      ...
      default:
        ...
        console.error('Available commands: check, fetch, download, search, mark-read, mark-unread, move, list-mailboxes');
        process.exit(1);
    }
