...
export async function runStatus(args) {
    ...
    if (!taskId) {
        console.error("Usage: cli-worker status <taskId> [--provider kimi|claude|opencode]");
        return 1;
    }
    ...
    const reportPath = path.join(worktreeDir, ".openclaw", provider.reportSubdir(), `${taskId}.json`);
    try {
