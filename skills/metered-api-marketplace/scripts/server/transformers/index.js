// Pure-function transformer registry.
// Each transformer must be deterministic and stateless: (input) -> output.
...
function simpleHashScore(s) {
  // Deterministic pseudo-score 0..100 from string content.
...
export const transformers = {
  'revenue-amplifier': (input) => {
    ...
    return {
      ...
      pricing: {
        strategy: '3-option anchored pricing with value gates',
        numbers: [99, 299, 999]
      },
      headlines: [
      ...
      risks: [
        'If promises are vague, conversion will be capped',
        'If proof is missing, CAC rises',
        'If onboarding is slow, churn spikes'
      ],
      next_steps: [
        'Add one primary claim + one proof asset above the fold',
        'Create a 3-step onboarding checklist',
        'Test two headline variants with the same offer'
      ],
      constraints
    };
  },
  ...
  'ad-copy-optimizer': (input) => {
    ...
    return {
      ...
      variants,
      compliance_notes: [
        'Avoid absolute guarantees unless you can prove them',
        'Add disclaimers for financial/health claims where required'
      ]
    };
  },
  ...
  'lead-scoring-engine': (input) => {
    ...
    if (employeeCount >= Number(icp.min_employees ?? 0) && employeeCount <= Number(icp.max_employees ?? 1e9)) {
      score += 20; reasons.push('Company size fits ICP');
    }
    ...
    const next_best_action = band === 'hot'
      ? 'Book a call; send 2-slot scheduling link.'
      : band === 'warm'
      ? 'Send a 3-question qualification email and one case study.'
      : 'Add to nurture; send value-first resource.';
  ...
  'competitor-price-monitor': (input) => {
    // Deterministic placeholder: real version would fetch/crawl. Here we do “analysis on provided snapshots”.
    ...
    const analyzed = items.map((it) => {
      ...
      return {
        ...
        key_features: features.slice(0, 6)
      };
    });
    ...
    return {
      ...
      summary: {
        average_price: avg,
        recommendation: avg ? 'Anchor slightly above average with stronger proof, or slightly below with tighter scope.' : 'Provide competitor prices to compute benchmarks.'
      }
    };
  },
  ...
  'contract-risk-analyzer': (input) => {
    ...
    for (const c of checks) {
      if (c.re.test(text)) flags.push({ flag: c.key, severity: c.severity });
    }
    ...
    return {
      risk_level: flags.some((f) => f.severity === 'high') ? 'high' : flags.length ? 'medium' : 'low',
      flags,
      negotiation_points: Array.from(new Set(negotiation_points)).slice(0, 10)
    };
  },
  ...
  'market-trend-signal-generator': (input) => {
    ...
    // Deterministic “signal framing” from provided inputs (no web calls)
    const signals = keywords.slice(0, 10).map((k) => ({
      keyword: k,
      signal: `If ${k} is increasing in your inbound/outbound conversations, build a dedicated offer page + 3 ads around it.`,
      confidence: ['low', 'medium', 'high'][simpleHashScore(k) % 3]
    }));
    ...
    return {
      ...
      signals,
      next_steps: [
        'Collect 20 recent customer quotes and map them to the keywords',
        'Run a 7-day test campaign per top 2 signals',
        'Promote the winning signal into your core positioning'
      ]
    };
  },
      ...
      emails: [
        {
          ...
          body: `Hey ${name} — saw ${trigger}. If you're trying to ${uvp}, we can help.\n\nOpen to a 10-min chat this week?`
        },
        {
          subject: `Worth a quick look?`,
          body: `Circling back — if ${company} is prioritizing ${uvp}, I can send a 1-page plan tailored to your situation. Want it?`
        }
      ],
  ...
  'seo-brief-content-plan': (input) => {
    ...
    return {
      ...
      brief: {
        ...
        internal_links: cluster.slice(1, 5)
      }
    };
  },
  ...
  'arbitrage-spread-detector': (input) => {
    ...
    for (const leg of legs) {
      ...
      if (Number.isFinite(buy) && Number.isFinite(sell) && buy > 0 && sell > 0 && sell > buy) {
        const spread = sell - buy;
        const pct = spread / buy;
        opps.push({
          symbol: asStr(leg.symbol, 'asset'),
          buy,
          sell,
          spread,
          spread_pct: Number(pct.toFixed(4)),
          confidence: pct > 0.05 ? 'high' : pct > 0.02 ? 'medium' : 'low'
        });
      }
    }
    ...
    return {
      ...
      notes: ['This is purely arithmetic on provided prices; add fees/slippage checks before live trading.']
    };
  },
  ...
  'churn-risk-retention-playbook': (input) => {
    ...
    return {
      risk,
      save_moves: risk === 'high'
        ? ['Offer fast human help', 'Reduce scope temporarily', 'Provide a clear success plan', 'Offer pause instead of cancel']
        : ['Send best-practice guide', 'Invite to office hours', 'Highlight unused features'],
  ...
  'support-triage-refund-risk': (input) => {
    ...
    const out = tickets.map((t) => {
      ...
      return {
        ...
        suggested_macro: urgent
          ? 'Apologize, confirm impact, provide immediate workaround + ETA.'
          : refund
          ? 'Acknowledge, gather details, offer resolution path, avoid blame.'
          : 'Answer question with steps + link to docs.'
      };
    });
    ...
    return {
      tickets: out,
      summary: {
        p0: out.filter((x) => x.priority === 'p0').length,
        p1: out.filter((x) => x.priority === 'p1').length,
        p2: out.filter((x) => x.priority === 'p2').length
      }
    };
  },
  ...
  'pricing-anchor': (input) => {
    ...
    return {
      ...
      justification_logic: [
        'Anchor frames the reference point; target feels reasonable by comparison',
        'Higher anchor implies completeness / premium outcome',
        'Use proof + specificity to defend the anchor'
      ],
      framing_copy: anchor
        ? [
            `Comparable solutions in ${market} routinely cost $${anchor}+ when you include time, tools, and mistakes.`,
            `We priced ${product} at $${targetPrice} because it removes the expensive uncertainty and compresses time-to-result.`
          ]
        : ['Provide product_price to compute anchor framing.']
    };
  },
  ...
  'objection-crush': (input) => {
    ...
    const angles = objections.slice(0, 3).map((o) => ({
      objection: o,
      counters: [
        `Reframe: ${o} is valid — which is why ${product} includes a step-by-step path and support.`,
        `Proof: add one measurable example or case study that specifically addresses "${o}".`,
        `Risk removal: add a conditional guarantee tied to implementing the steps.`
      ],
    ...
    return {
      ...
      objection_playbook: angles,
      guarantee_suggestions: [
        'Implementation-based guarantee ("Do X steps, if no progress, we fix it")',
        'Milestone guarantee ("Hit milestone by day N or extend for free")'
      ]
    };
  },
  ...
  'offer-stack': (input) => {
    ...
    const bonuses = [
      'Fast-start checklist (first 48 hours)',
      'Templates/swipe file pack',
      'Implementation review (15-min async audit)',
      'Risk reversal / guarantee upgrade',
      'Private community or office hours (limited seats)'
    ];
    ...
    return {
      ...
      industry,
      target_buyer: buyer,
      perceived_value_multipliers: bonuses,
  ...
  'cta-boost': (input) => {
    ...
    const urgency = stage.toLowerCase().includes('bottom') || stage.toLowerCase().includes('purchase')
      ? ['(today)', '(before prices change)', '(limited slots)']
      : ['(free)', '(2 min)', '(instant)'];
  ...
  'headline-power-scorer': (input) => {
    ...
    return {
      ...
      example_upgrades: headline
        ? [
            `How to ${headline} (in 7 days)`,
            `${headline} — the 3-step system`,
            `Get ${headline} without the usual mess`
          ]
        : []
    };
  },
  ...
  'competitive-undercut': (input) => {
    ...
    return {
      ...
      notes: ['This is deterministic math. Validate with churn/CAC before changing pricing.']
    };
  },
  ...
  'scarcity-clock': (input) => {
    ...
    return {
      ...
      duration,
      scarcity_language: [
        `Enrollment closes in ${duration}.`,
        'Capacity is capped to protect delivery quality.',
        'Bonuses disappear at the deadline (not later).'
      ],
  ...
  'risk-reversal': (input) => {
    ...
    return {
      ...
      guarantee_models: [
        'Time-boxed refund guarantee (7/14/30 days)',
        'Milestone guarantee (hit X by day N or we extend/support free)',
        'Implementation guarantee (do the steps, we fix gaps)',
        'Performance credit (partial refund as credits)'
      ],
  ...
  'upsell-ladder': (input) => {
    ...
    return {
      ...
      ladder: [
        { tier: 'core', price: tier1, value_gate: 'DIY + templates' },
        { tier: 'pro', price: tier2, value_gate: 'Done-with-you support + reviews' },
        { tier: 'premium', price: tier3, value_gate: 'Done-for-you implementation / concierge' }
      ],
      profit_projection_simple: {
        example_100_buyers: { core: 100 * tier1, pro_if_20pct: 20 * tier2, premium_if_5pct: 5 * tier3 }
      }
    };
  },
  ...
  'authority-positioning': (input) => {
    ...
    return {
      ...
      product,
      positioning_angles: [
        'Operator-led: built from real execution, not theory',
        'Contrarian: avoid the common mistake everyone makes',
        'Method: name a repeatable framework',
        'Proof stack: quantify outcomes + show artifacts'
      ],
  ...
  'simple-arbitrage-signal': (input) => {
    ...
    return {
      ...
      fees: fees || 0,
      net_margin: Number.isFinite(net) ? Number(net.toFixed(4)) : null,
      go_no_go: go ? 'go' : 'no-go',
      breakeven_sell_price: Number.isFinite(breakevenSell) ? Number(breakevenSell.toFixed(4)) : null
    };
  },
  ...
  'funnel-leak-detector': (input) => {
    ...
    // Identify highest leverage fix: whichever rate is lowest relative to reasonable baselines.
    ...
    return {
      ...
      math: {
        leads: Number.isFinite(leads) ? Number(leads.toFixed(2)) : null,
        customers: Number.isFinite(customers) ? Number(customers.toFixed(2)) : null,
        revenue: Number.isFinite(revenue) ? Number(revenue.toFixed(2)) : null
      },
      biggest_leak: biggestLeak,
      recommended_fix: biggestLeak === 'opt_in_rate'
        ? ['Improve lead magnet + headline', 'Reduce form friction', 'Add proof above the opt-in']
        : ['Tighten offer + risk reversal', 'Improve checkout friction', 'Add urgency + clearer CTA'],
      note: 'Pure math + heuristic baselines; replace baselines with your actual benchmarks.'
    };
  }
};
