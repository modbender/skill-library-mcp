...
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "-1003892992445")
...
# Global for tracking person detection cooldown
LAST_PERSON_TIME = 0

def mark_person_detected():
    """Call this when analysis confirms a person - extends cooldown"""
    global LAST_PERSON_TIME
    LAST_PERSON_TIME = time.time()
    log("👤 Person confirmed - entering extended cooldown (10min)")
...
def update_stream():
    ...
    while os.path.exists(ACTIVE_FILE):
        ...
        time.sleep(5)  # Reduced from 2s to 5s - less camera activity
...
def main():
    ...
    last_alert = 0
    global LAST_PERSON_TIME
    MOTION_COOLDOWN = 120       # 2 min between any motion alerts
    PERSON_COOLDOWN = 600       # 10 min after detecting a person
    
    try:
        while os.path.exists(ACTIVE_FILE):
            time.sleep(1)  # Check every 1s instead of 0.5s
            
            if detect_motion():
                now = time.time()
                
                # Check person cooldown first (longer)
                if now - LAST_PERSON_TIME < PERSON_COOLDOWN:
                    remaining = int(PERSON_COOLDOWN - (now - LAST_PERSON_TIME))
                    log(f"😴 Person recently detected, chilling ({remaining}s left)")
                    continue
                
                # Check general motion cooldown
                if now - last_alert < MOTION_COOLDOWN:
                    remaining = int(MOTION_COOLDOWN - (now - last_alert))
                    log(f"⏳ Motion in cooldown ({remaining}s)")
                    continue
                ...
                # Note: LAST_PERSON_TIME updated when user replies "person"
                # or when analysis confirms a person
