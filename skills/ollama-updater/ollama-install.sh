...
# Modified version with resumable download support (断点续传)
# Original: https://ollama.com/install.sh
# Modified: 2026-02-20 - Added resumable download with curl -C -
...
if [ "$OS" = "Darwin" ]; then
    ...
    # 使用 -C - 实现断点续传
    curl --fail --show-error --location --progress-bar -C - \
...
download_file() {
    ...
    while [ $retry_count -lt $max_retries ]; do
        ...
        # 使用 -C - 实现断点续传
        if curl --fail --show-error --location --progress-bar -C - -o "$output" "$url" 2>&1; then
            return 0
        fi
...
download_and_extract() {
  ...
  - Arch: sudo pacman -S zstd"
        ...
        status "Downloading ${filename}.tar.zst (with resumable support)"
        download_file "${url_base}/${filename}.tar.zst${VER_PARAM}" "$TEMP_DIR/download.tar.zst"
    ...
    status "Downloading ${filename}.tgz (with resumable support)"
    download_file "${url_base}/${filename}.tgz${VER_PARAM}" "$TEMP_DIR/download.tar.gz"
    
    status "Extracting..."
    $SUDO tar -xzf "$TEMP_DIR/download.tar.gz" -C "${dest_dir}"
}
