...
export async function getAccessToken(profile, scopes) {
  ...
  if (!fs.existsSync(cfgPath)) throw new Error(`Missing profile config: ${cfgPath} (run setup.mjs)`);
  if (!fs.existsSync(cachePath)) throw new Error(`Missing token cache: ${cachePath} (run setup.mjs)`);

  const cfg = readJson(cfgPath);
  const cacheText = fs.readFileSync(cachePath, 'utf8');

  // Fallback: raw OAuth token JSON
  try {
  ...
  if (!accounts.length) throw new Error(`No accounts in token cache for profile ${profile} (re-run setup.mjs)`);
