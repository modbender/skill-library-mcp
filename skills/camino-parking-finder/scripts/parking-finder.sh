#!/bin/bash
# Camino AI Parking Finder - Locate parking garages, lots, and street parking
# Usage: ./parking-finder.sh '{"query": "parking near Madison Square Garden", "limit": 10}'
...
if [ -z "$1" ]; then
    ...
    echo "Usage: ./parking-finder.sh '{\"query\": \"parking near Madison Square Garden\", \"limit\": 10}'" >&2
    exit 1
fi
...
build_query_string() {
    # Use provided query or default to "parking garages lots"
    local query=$(echo "$INPUT" | jq -r '.query // "parking garages lots"')
    ...
    local radius=$(echo "$INPUT" | jq -r '.radius // "1000"')
