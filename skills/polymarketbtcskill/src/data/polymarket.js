...
export async function fetchBtc15mMarketSnapshot() {
  const cfg = CONFIG.polymarket;

  let market = null;
  if (cfg.marketSlug) {
    market = await fetchMarketBySlug(cfg.marketSlug);
  } else if (cfg.autoSelectLatest) {
    const events = await fetchLiveEventsBySeriesId({ seriesId: cfg.seriesId, limit: 25 });
  ...
  for (let i = 0; i < outcomes.length; i += 1) {
    ...
    if (!tokenId) continue;
    if (label.toLowerCase() === cfg.upOutcomeLabel.toLowerCase()) upTokenId = tokenId;
    if (label.toLowerCase() === cfg.downOutcomeLabel.toLowerCase()) downTokenId = tokenId;
  }

  const upIndex = outcomes.findIndex((x) => String(x).toLowerCase() === cfg.upOutcomeLabel.toLowerCase());
  const downIndex = outcomes.findIndex((x) => String(x).toLowerCase() === cfg.downOutcomeLabel.toLowerCase());
