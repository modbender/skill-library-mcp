...
case "$cmd" in
  desk)
    # Desk lamp shortcut: ha.sh desk on|off|toggle [brightness]
    action="${1:-state}"
    shift || true
    entity="light.wiz_rgbw_tunable_1173fc"
    case "$action" in
      on)
        brightness="${1:-}"
        if [[ -n "$brightness" ]]; then
          api -X POST "$HA_URL/api/services/light/turn_on" \
            -d "{\"entity_id\": \"$entity\", \"brightness\": $brightness}" >/dev/null
        else
          api -X POST "$HA_URL/api/services/light/turn_on" \
            -d "{\"entity_id\": \"$entity\"}" >/dev/null
        fi
        echo "✓ desk lamp turned on"
        ;;
      off)
        api -X POST "$HA_URL/api/services/light/turn_off" \
          -d "{\"entity_id\": \"$entity\"}" >/dev/null
        echo "✓ desk lamp turned off"
        ;;
      toggle)
        api -X POST "$HA_URL/api/services/light/toggle" \
          -d "{\"entity_id\": \"$entity\"}" >/dev/null
        echo "✓ desk lamp toggled"
        ;;
      state|*)
  ...
  ceiling)
    # Ceiling light shortcut: ha.sh ceiling on|off|toggle [brightness]
    action="${1:-state}"
    shift || true
    entity="light.wiz_rgbw_tunable_44a6a8"
    case "$action" in
      on)
        brightness="${1:-}"
        if [[ -n "$brightness" ]]; then
        ...
        echo "✓ ceiling light turned on"
        ;;
      off)
        api -X POST "$HA_URL/api/services/light/turn_off" \
          -d "{\"entity_id\": \"$entity\"}" >/dev/null
        echo "✓ ceiling light turned off"
        ;;
      toggle)
        api -X POST "$HA_URL/api/services/light/toggle" \
          -d "{\"entity_id\": \"$entity\"}" >/dev/null
        echo "✓ ceiling light toggled"
        ;;
      state|*)
        api "$HA_URL/api/states/$entity" | jq -r '.state // "unknown"'
        ;;
...
Commands:
  desk [on|off|toggle|state] [brightness]     Desk lamp shortcut
  ceiling [on|off|toggle|state] [brightness]  Ceiling light shortcut
