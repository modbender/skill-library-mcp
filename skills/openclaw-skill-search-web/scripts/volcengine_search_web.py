...
def search_web(question: str, stream: bool = False):
    ...
    # Get API key
    api_key = os.getenv("VOLCENGINE_SEARCH_API_KEY") or os.getenv("VOLCENGINE_ARK_API_KEY")

    # Get bot ID
    bot_id = os.getenv("VOLCENGINE_SEARCH_BOT_ID")
    if not bot_id:
        print("VOLCENGINE_SEARCH_BOT_ID environment variable is required.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  VOLCENGINE_SEARCH_BOT_ID      (required) - Bot ID from console")
        print("  VOLCENGINE_SEARCH_API_KEY or VOLCENGINE_ARK_API_KEY - API key for authentication")
        sys.exit(1)
