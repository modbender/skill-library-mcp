...
[ ! -d "$SITE_DIR" ] && { echo "❌ No audits found for $SITE. Run an audit first."; exit 1; }
...
if not audits:
    print(f"❌ No audit data found for {site}")
    exit(1)

# Build report
report = f"# SEO Audit Report — {site}\n"
...
if "geo" in audits:
    ...
    if recs:
        report += "## 💡 GEO Optimization Recommendations\n\n"
        for r in recs:
            report += f"1. {r}\n"
        report += "\n"
