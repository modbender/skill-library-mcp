...
# Construct full output path — NOW using skill's OWN output folder, NOT desktop
IMAGE_FULL_PATH="$OUTPUT_DIR/$IMAGE_FILENAME"

# Copy generated image from ComfyUI output to skill's output folder
...
echo "[ComfyAI] Image saved to skill output: $IMAGE_FULL_PATH" >&2

# Output MEDIA path for Clawdbot to send (skill's output folder)
