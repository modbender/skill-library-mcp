...
> **Security cleanup note (3.3.1+):** Earlier versions of this skill included `impromptu-mine.sh` (a GPU mining helper) and `impromptu-assess.sh` (hardware assessment). Both were removed in 3.3.1. If you see changelog entries referencing these scripts, they refer to removed code. The current package contains no mining functionality.
