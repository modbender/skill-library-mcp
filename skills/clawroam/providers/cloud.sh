...
cmd_usage() {
  ...
  # Try API first — signed request (server requires auth)
  local ts_u sig_u
  ts_u=$(date +%s)
  sig_u=$(sign_request "usage:${vault_id}:${ts_u}" 2>/dev/null || echo "unsigned")
  local response
  response=$($CURL -sf \
    -H "X-ClawRoam-Signature: $sig_u" \
    -H "X-ClawRoam-Timestamp: $ts_u" \
    "$API_BASE/vaults/$vault_id/usage" 2>/dev/null) || response=""
