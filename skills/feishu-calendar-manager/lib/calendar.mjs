...
import { ensureAdminIncluded, getDisplayName } from './employees.mjs';
import { getCalendarId as discoverCalendarId } from './config.mjs';

// Fallback calendar ID (used only if auto-discovery fails)
export const FALLBACK_CALENDAR_ID = '<primary-calendar-id>';
...
/**
 * Get calendar ID (with auto-discovery)
 */
export async function getCalendarId() {
  ...
  if (process.env.FEISHU_CALENDAR_ID) {
    cachedCalendarId = process.env.FEISHU_CALENDAR_ID;
    return cachedCalendarId;
  }
...
/**
 * Reset cached calendar ID (useful for testing)
 */
export function resetCalendarIdCache() {
 ...
 * @param {string} [options.calendarId] - Calendar ID (auto-discovered if not provided)
...
export async function createEvent({
  ...
  // Auto-discover calendar ID if not provided
  const targetCalendarId = calendarId || await getCalendarId();

  // Always include Boyang
  const finalAttendeeIds = ensureAdminIncluded(attendeeIds);
  ...
  // Create the event
  const eventData = await larkApi('POST', `/calendar/v4/calendars/${targetCalendarId}/events`, {
    data: {
      summary: title,
  ...
  if (finalAttendeeIds.length > 0) {
    const attendeesData = await addEventAttendees(targetCalendarId, event.event_id, finalAttendeeIds);
...
export async function updateEvent({
  ...
  const result = await larkApi('PATCH', `/calendar/v4/calendars/${targetCalendarId}/events/${eventId}`, {
    data: updateData
  });
 ...
 * @param {string} [calendarId] - Calendar ID (auto-discovered if not provided)
 * @param {boolean} [notify] - Send notification to attendees
 * @returns {boolean} - Success
 */
export async function deleteEvent(eventId, calendarId = null, notify = true) {
  // Auto-discover calendar ID if not provided
  const targetCalendarId = calendarId || await getCalendarId();

  await larkApi('DELETE', `/calendar/v4/calendars/${targetCalendarId}/events/${eventId}`, {
...
export async function listEvents({
  ...
  const result = await larkApi('GET', `/calendar/v4/calendars/${targetCalendarId}/events`, {
    params: {
 ...
 */
export async function getEvent(eventId, calendarId = null) {
  // Auto-discover calendar ID if not provided
  const targetCalendarId = calendarId || await getCalendarId();

  const result = await larkApi('GET', `/calendar/v4/calendars/${targetCalendarId}/events/${eventId}`);
  return result.event;
}

// Backward compatibility: export old name
export { ensureAdminIncluded as ensureBoyangIncluded } from './employees.mjs';
