#!/bin/bash
# Gateway Watchdog v4.4 - OAuth 토큰 만료 자동 감지 및 복구
#
# v4.4 개선사항:
# - check_oauth_expired: gateway.err.log에서 최근 15분 내 401 OAuth 만료 에러 감지
# - HTTP OK 상태에서도 agent 인증 실패 시 자동 재시작 (기존 watchdog이 탐지 못하던 케이스)
# - auth 재시작 쿨다운 1시간 (연쇄 재시작 방지)
# - Discord + ntfy 알림: 토큰 미갱신 시 /login 안내 포함
#
# v4.3 개선사항:
# - preflight_check: launchctl 미관리 + 포트 점유 = 좀비 감지 → 자동 kill
# - STOPPED:exit_* + HTTP 200이어도 좀비가 응답하는 경우 "정상 스킵" 오판 방지
# - 좀비 종료 후 launchctl kickstart로 즉시 정상 재기동
#
# v4.2 개선사항:
...
AUTH_COOLDOWN_FILE="$STATE_DIR/auth-expire-last-restart"
...
AUTH_EXPIRE_DETECT_MINUTES=15 # OAuth 만료 에러 감지 윈도우 (분)
AUTH_EXPIRE_COOLDOWN_SECS=3600 # OAuth 재시작 쿨다운 (1시간)
...
preflight_check() {
    ...
    # 3. 포트 충돌 확인 (좀비 프로세스 감지 및 자동 종료 포함)
    local port_user
    port_user=$(lsof -i ":$GATEWAY_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)
    local gateway_pid
    gateway_pid=$(launchctl list 2>/dev/null | grep "$LAUNCHD_SERVICE" | awk '{print $1}' | grep -v "^-$")

    if [[ -n "$port_user" ]] && [[ "$port_user" != "$gateway_pid" ]]; then
        if [[ -z "$gateway_pid" ]]; then
            # launchctl에 PID 없음 + 포트 점유 중 = 좀비 프로세스 (v4.3 핵심 수정)
            log "WARN" "좀비 게이트웨이 감지 (PID: $port_user) — launchctl 미관리, 포트 강제 해제"
            if ! $DRY_RUN; then
                kill "$port_user" 2>/dev/null || true
                sleep 2
                # 아직 살아있으면 SIGKILL
                if kill -0 "$port_user" 2>/dev/null; then
                    log "WARN" "SIGTERM 무시됨 — SIGKILL 전송 (PID: $port_user)"
                    kill -9 "$port_user" 2>/dev/null || true
                    sleep 1
                fi
                log "ACTION" "좀비 프로세스 종료 완료 (PID: $port_user)"
                # launchctl kickstart로 즉시 정상 재기동
                launchctl kickstart "gui/$(id -u)/$LAUNCHD_SERVICE" 2>/dev/null || \
                    launchctl bootstrap "gui/$(id -u)" "$LAUNCHD_PLIST" 2>/dev/null || true
                log "ACTION" "launchctl kickstart 실행 (좀비 제거 후 재기동)"
            else
                log "DRY-RUN" "좀비 kill 생략 (PID: $port_user)"
            fi
            issues+=("좀비 프로세스 종료 (PID: $port_user)")
        else
            log "WARN" "포트 $GATEWAY_PORT가 다른 프로세스(PID: $port_user)에 의해 사용 중 (gateway PID: $gateway_pid)"
            issues+=("포트 충돌")
        fi
    fi
...
try:
    # doctor --fix 출력에서 제거 대상 키 파싱
    result = subprocess.run(['openclaw', 'doctor', '--fix'], capture_output=True, text=True)
    doctor_out = result.stdout + result.stderr
    # '  - key.path' 패턴에서 키 경로 추출
    parsed_keys = re.findall(r'^\s+-\s+([a-zA-Z0-9_.]+)\s*$', doctor_out, re.MULTILINE)

    # 하드코딩 fallback 목록 (알려진 invalid 키)
    known_invalid = {
        'channels.discord': ['streaming', 'statusReactions'],
        'tools': ['mcp'],
    }
    ...
    # fallback: 하드코딩 목록도 처리
    for section, keys in known_invalid.items():
        parts = section.split('.')
        obj = c
        try:
            for p in parts:
...
# ============================================================================
# v4.4 신규: OAuth 토큰 만료 감지
# ============================================================================

check_oauth_expired() {
    [[ ! -f "$GATEWAY_ERR_LOG" ]] && return 1
    local cutoff_epoch
    cutoff_epoch=$(date -v "-${AUTH_EXPIRE_DETECT_MINUTES}M" +%s 2>/dev/null) || return 1
    local found=0
    while IFS= read -r line; do
        ...
        [[ -z "$ts" ]] && continue
        local line_epoch
        line_epoch=$(date -j -f "%Y-%m-%dT%H:%M:%S" "$ts" +%s 2>/dev/null) || continue
        if [[ $line_epoch -ge $cutoff_epoch ]]; then
            found=1
            break
        fi
    done < <(grep "OAuth token has expired" "$GATEWAY_ERR_LOG" 2>/dev/null | tail -20)
    [[ $found -eq 1 ]]
}
...
is_in_auth_cooldown() {
    ...
    local last
    last=$(cat "$AUTH_COOLDOWN_FILE" 2>/dev/null || echo "0")
    local elapsed=$(( $(date +%s) - last ))
    [[ $elapsed -lt $AUTH_EXPIRE_COOLDOWN_SECS ]]
}
...
if [[ "$pid_status" == "NOT_LOADED" ]] || [[ "$pid_status" == STOPPED:* ]] || [[ "$pid_status" == CRASHED:* ]]; then
    ...
    if [[ "$_precheck_http" == "OK" ]]; then
        ...
        if [[ "$pid_status" == CRASHED:signal_* ]]; then
            ...
            # v4.3: STOPPED/NOT_LOADED + HTTP 200 → 좀비 여부 검증
            # CRASHED:signal_* 과 달리, STOPPED:exit_* 은 child process 생존 시나리오 없음
            local _port_holder
            _port_holder=$(lsof -i ":$GATEWAY_PORT" -sTCP:LISTEN -t 2>/dev/null | head -1)
            local _lctl_pid
            _lctl_pid=$(launchctl list 2>/dev/null | grep "$LAUNCHD_SERVICE" | awk '{print $1}' | grep -v "^-$")
            if [[ -n "$_port_holder" ]] && [[ -z "$_lctl_pid" ]]; then
                # 좀비 확인 — preflight에서 처리됐어야 하나 재진입 방어
                log "WARN" "STOPPED + HTTP 200 이나 포트 점유자(PID: $_port_holder)가 launchctl 미관리 — 좀비로 판정, 복구 진행"
                rm -f "$ORPHAN_SINCE_FILE"
                # 아래 재시작 로직으로 fall-through (exit 하지 않음)
            else
                rm -f "$ORPHAN_SINCE_FILE"
...
if [[ "$http_status" == "OK" ]]; then
    ...
    # v4.4: OAuth 토큰 만료 감지 (HTTP OK이지만 agent 401 에러)
    if check_oauth_expired; then
        if ! is_in_auth_cooldown; then
            log "WARN" "OAuth 토큰 만료 감지 — gateway 재시작 시도"
            set_auth_restart
            request_restart "OAuth 토큰 만료 (auth 401)"
            send_alert "warning" "OAuth 토큰 만료 감지" \
                "Anthropic OAuth 토큰 만료로 gateway를 재시작합니다.\n토큰이 갱신되지 않은 경우 /login 후 재시작 필요" \
                "[{\"name\":\"원인\",\"value\":\"OAuth token expired (HTTP 401)\",\"inline\":true},{\"name\":\"쿨다운\",\"value\":\"${AUTH_EXPIRE_COOLDOWN_SECS}초\",\"inline\":true}]"
            log "INFO" "========== 체크 완료 (OAuth 재시작) =========="
            exit 0
        else
            log "INFO" "OAuth 만료 감지 — auth 재시작 쿨다운 중 (${AUTH_EXPIRE_COOLDOWN_SECS}초)"
        fi
    fi
