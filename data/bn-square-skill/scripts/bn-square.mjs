...
// src/utils/errors.ts
var KEY_VALUE_PATTERN = /\b([A-Za-z0-9_-]*(?:cookie|token|session|csrf|authorization)[A-Za-z0-9_-]*)=([^;\s,]+)/gi;
var BEARER_PATTERN = /\b(Bearer\s+)([A-Za-z0-9\-._~+/]+=*)/gi;
var REDACTED = "<redacted>";
var sanitizeSensitiveText = (text) => {
  const sanitizedKeyValue = text.replace(KEY_VALUE_PATTERN, (_, key) => `${key}=${REDACTED}`);
  return sanitizedKeyValue.replace(BEARER_PATTERN, (_, prefix) => `${prefix}${REDACTED}`);
};
...
var SessionExpiredError = class extends AppError {
  constructor(message = "Session expired or unauthorized", status, cause) {
    super({
...
var RateLimitError = class extends AppError {
  constructor(message, status = 429, cause) {
    super({
...
var toSafeErrorMessage = (error, fallback = "Unexpected error") => {
...
// src/api/client.ts
var textResponseSchema = external_exports.string();
var isAbortError = (error) => error instanceof DOMException ? error.name === "AbortError" : false;
...
var getErrorPreview = async (response) => {
  try {
...
var BinanceHttpClient = class {
  ...
  async postJson(path3, body, options = {}) {
    return this.request({
      ...options,
      method: "POST",
      path: path3,
  ...
  async postFormData(path3, formData, options = {}) {
    return this.request({
      ...options,
      method: "POST",
      path: path3,
  ...
  async request(options) {
    ...
    const hasJsonBody = options.body !== void 0 && !options.formData;
    ...
    try {
      ...
      throw new NetworkError(
        `Network request failed for ${options.method} ${options.path}: ${error instanceof Error ? sanitizeSensitiveText(error.message) : "Unknown error"}`,
        error
      );
    } finally {
...
var BINANCE_SQUARE_URL = "https://www.binance.com/en/square";
var CDP_TIMEOUT_MS = 15e3;
var parseCdpBaseUrl = (cdpUrl) => {
  const parsed = new URL(cdpUrl);
  return `${parsed.protocol}//${parsed.host}`;
};
var fetchCdpPages = async (cdpUrl) => {
...
var createCdpConnection = (wsUrl, timeoutMs) => new Promise((resolve, reject) => {
...
var escapeForJs = (value) => value.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/\n/g, "\\n").replace(/\r/g, "\\r");
var injectCookies = async (send, cookieHeader) => {
  const segments = cookieHeader.split(";").map((s) => s.trim()).filter((s) => s.length > 0);
...
var navigateAndWait = async (send, url) => {
...
var buildFetchExpression = (path3, method, bodyExpression, csrfToken) => {
  const headers = {
  ...
  return `
    (async () => {
      const resp = await fetch('${escapeForJs(path3)}', {
        method: '${method}',
        headers: ${headersJson},
        credentials: 'include',
        body: ${bodyExpression}
...
var buildFormDataFetchExpression = (path3, fieldName, base64Data, fileName, mimeType, csrfToken) => {
...
var evaluateFetch = async (send, expression, timeoutMs) => {
  ...
  if (exceptionDetails) {
    const exceptionText = exceptionDetails.exception?.description ?? exceptionDetails.text ?? "Unknown browser error";
...
var handleFetchResult = (fetchResult, method, path3) => {
  const { status, body } = fetchResult;
  if (status === 401 || status === 403) {
    const preview = body.length > 240 ? `${body.slice(0, 240)}...` : body;
    throw new SessionExpiredError(
      `Request failed (${status}) for ${method} ${path3}: ${sanitizeSensitiveText(preview)}`,
      status
    );
  }
  if (status === 429) {
    throw new RateLimitError(
      `Request failed (${status}) for ${method} ${path3}`,
      status
    );
  }
...
var BrowserClient = class {
  ...
  disconnect() {
    if (this.cdpClose) {
      this.cdpClose();
      this.cdpClose = void 0;
      this.cdpSend = void 0;
    }
  }
  async getJson(path3, options) {
    const fullPath = this.buildPath(path3, options?.query);
    ...
    return handleFetchResult(result, "GET", path3);
  }
  async postJson(path3, body) {
    ...
    const result = await evaluateFetch(this.getSend(), expression, this.config.requestTimeoutMs);
    return handleFetchResult(result, "POST", path3);
  }
  async postFormData(path3, formData) {
...
var COOKIE_SEGMENT_PATTERN = /^[^=\s;]+=[^;]+$/;
var DEFAULT_REFERER = "https://www.binance.com/en/square";
var DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36";
var DEFAULT_POST_URL_TEMPLATE = "https://www.binance.com/en/square/post/{postId}";
var endpointConfigSchema = external_exports.object({
...
var imageConfigSchema = external_exports.object({
  uploadFieldName: nonEmptyString.default("file"),
  maxBytes: external_exports.number().int().positive().default(5 * 1024 * 1024),
  allowedMimeTypes: external_exports.array(nonEmptyString).min(1).default(["image/jpeg", "image/png", "image/webp"])
});
var binanceConfigSchema = external_exports.object({
  cdpUrl: external_exports.string().url().optional(),
  apiBaseUrl: external_exports.string().url().default("https://www.binance.com"),
  ...
  referer: external_exports.string().url().default(DEFAULT_REFERER),
  requestTimeoutMs: external_exports.number().int().positive().default(1e4),
  ...
  const segments = value.cookieHeader.split(";").map((segment) => segment.trim()).filter((segment) => segment.length > 0);
...
var parseOptionalNumber = (rawValue) => {
...
var parseOptionalCsv = (rawValue) => {
  if (rawValue === void 0 || rawValue.trim().length === 0) {
    return void 0;
  }
  const parsed = rawValue.split(",").map((item) => item.trim()).filter((item) => item.length > 0);
  return parsed.length > 0 ? parsed : void 0;
};
var resolveConfigPath = (cwd, options, env) => {
...
var toValidationErrorMessage = (error) => error.issues.map((issue) => {
  const issuePath = issue.path.length > 0 ? issue.path.join(".") : "config";
  return `${issuePath}: ${issue.message}`;
}).join("; ");
var readEnv = (options, cwd) => {
  if (options.env) {
    return options.env;
  }
  ...
  const parsedEnv = import_dotenv.default.parse(rawEnv);
  return {
    ...parsedEnv,
    ...envFromProcess
  };
};
var buildConfigFromEnv = (env) => {
  const configShape = {
...
var loadConfig = (options = {}) => {
  ...
  const root = typeof parsedJson === "object" && parsedJson !== null ? parsedJson : void 0;
  const normalizedConfigCandidate = root && "binance" in root && typeof root.binance === "object" && root.binance !== null ? root.binance : parsedJson;
  try {
...
var getEndpointPath = (config, key) => {
  switch (key) {
...
var buildPostStatusQuery = (config, postId) => ({
  [config.endpoints.statusQueryParam]: postId
});
var resolvePostUrl = (config, postId) => config.postUrlTemplate.replace("{postId}", encodeURIComponent(postId));

// src/api/types.ts
var unknownRecordSchema = external_exports.record(external_exports.unknown());
var binanceEnvelopeSchema = external_exports.object({
  ...
  msg: external_exports.string().optional(),
...
var isEnvelopeSuccess = (envelope) => {
...
var extractEnvelopeMessage = (envelope) => envelope.message?.trim() || envelope.msg?.trim() || "Binance API request failed";
var asRecord = (value) => {
...
var extractStringCandidate = (source, keys) => {
...
var extractDataRecord = (envelope) => asRecord(envelope.data);
var validateSessionResultSchema = external_exports.object({
  valid: external_exports.boolean(),
  userId: external_exports.string().optional(),
  username: external_exports.string().optional(),
  error: external_exports.string().optional()
});
var pollInputSchema = external_exports.object({
  question: external_exports.string().trim().min(1).max(140),
  options: external_exports.array(external_exports.string().trim().min(1).max(100)).min(2).max(4),
  durationHours: external_exports.number().int().min(1).max(168).default(24)
});
var publishPostInputSchema = external_exports.object({
  content: external_exports.string().trim().min(1).max(1e4),
  imageUrls: external_exports.array(external_exports.string().url()).max(9).optional(),
...
var publishPostResultSchema = external_exports.object({
  success: external_exports.boolean(),
  postId: external_exports.string().optional(),
  postUrl: external_exports.string().url().optional(),
  error: external_exports.string().optional()
});
var getPostStatusInputSchema = external_exports.object({
  postId: external_exports.string().trim().min(1)
});
var postStatusSchema = external_exports.union([
  external_exports.literal("published"),
  external_exports.literal("pending_review"),
  external_exports.literal("deleted"),
  external_exports.literal("not_found")
]);
var getPostStatusResultSchema = external_exports.object({
  status: postStatusSchema,
  postUrl: external_exports.string().url().optional()
});

// src/tools/get-post-status.ts
var isNotFoundSignal = (code, message) => {
  const normalizedCode = code?.toLowerCase();
  const normalizedMessage = message?.toLowerCase();
  return normalizedCode === "404" || normalizedCode === "not_found" || normalizedCode === "post_not_found" || normalizedMessage?.includes("not found") === true;
};
var mapStatus = (statusValue) => {
...
var getPostStatus = async (input, dependencies) => {
  ...
  if (!envelopeResult.success) {
    ...
    const postUrl2 = extractStringCandidate(fallback, ["postUrl", "url", "shareUrl"]) ?? resolvePostUrl(dependencies.config, parsedInput.postId);
    return getPostStatusResultSchema.parse({
      status: fallbackStatus,
      ...fallbackStatus === "not_found" ? {} : { postUrl: postUrl2 }
    });
  }
  ...
  const postUrl = extractStringCandidate(postData, ["postUrl", "url", "shareUrl"]) ?? extractStringCandidate(data, ["postUrl", "url", "shareUrl"]) ?? resolvePostUrl(dependencies.config, parsedInput.postId);
  return getPostStatusResultSchema.parse({
    status,
    postUrl
  });
...
var inferFileName = (imageUrl, mimeType) => {
  try {
    const parsedUrl = new URL(imageUrl);
    const candidateName = path2.basename(parsedUrl.pathname);
...
var extractUploadResultUrl = (rawResponse) => {
  const envelopeResult = binanceEnvelopeSchema.safeParse(rawResponse);
  if (envelopeResult.success) {
    ...
    return extractStringCandidate(data, ["url", "imageUrl", "imageURL", "location"]) ?? extractStringCandidate(nestedImage, ["url", "imageUrl", "imageURL", "location"]) ?? extractStringCandidate(asRecord(rawResponse) ?? {}, ["url", "imageUrl", "imageURL", "location"]);
  }
  const fallbackRecord = asRecord(rawResponse) ?? {};
  return extractStringCandidate(fallbackRecord, ["url", "imageUrl", "imageURL", "location"]);
};
var uploadImageFromUrl = async (imageUrl, dependencies) => {
  let parsedUrl;
  try {
...
// src/tools/publish-post.ts
var buildPayload = (input, uploadedImageUrls) => ({
...
var extractPublishResult = (config, rawResponse) => {
  const envelopeResult = binanceEnvelopeSchema.safeParse(rawResponse);
  ...
  if (envelopeResult.success) {
    ...
    const data = extractDataRecord(envelopeResult.data) ?? {};
    const postData = asRecord(data.post) ?? asRecord(data.article) ?? data;
    const postId = extractStringCandidate(postData, ["postId", "id", "articleId", "feedId"]) ?? extractStringCandidate(data, ["postId", "id", "articleId", "feedId"]);
    const postUrl = extractStringCandidate(postData, ["postUrl", "url", "shareUrl"]) ?? extractStringCandidate(data, ["postUrl", "url", "shareUrl"]) ?? (postId ? resolvePostUrl(config, postId) : void 0);
    return publishPostResultSchema.parse({
      success: true,
      ...postId ? { postId } : {},
      ...postUrl ? { postUrl } : {}
  ...
  const fallbackPostUrl = extractStringCandidate(rootRecord, ["postUrl", "url", "shareUrl"]) ?? (fallbackPostId ? resolvePostUrl(config, fallbackPostId) : void 0);
  return publishPostResultSchema.parse({
    success: true,
...
var publishPost = async (input, dependencies) => {
  try {
    const parsedInput = publishPostInputSchema.parse(input);
    const uploadImage = dependencies.uploadImage ?? ((imageUrl) => uploadImageFromUrl(imageUrl, {
      client: dependencies.client,
      config: dependencies.config
    }));
    const imageUrls = parsedInput.imageUrls ?? [];
    const uploadedImageUrls = imageUrls.length > 0 ? await Promise.all(imageUrls.map((imageUrl) => uploadImage(imageUrl))) : [];
...
// src/tools/validate-session.ts
var validateSession = async (dependencies) => {
  ...
  try {
    ...
    const userId = extractStringCandidate(data, ["userId", "uid", "id"]) ?? extractStringCandidate(nestedUser, ["userId", "uid", "id"]);
    const username = extractStringCandidate(data, ["username", "nickName", "nickname"]) ?? extractStringCandidate(nestedUser, ["username", "nickName", "nickname"]);
    return validateSessionResultSchema.parse({
      valid: true,
      ...userId ? { userId } : {},
      ...username ? { username } : {}
...
// src/index.ts
var createHttpToolClient = (config) => {
  const httpClient = new BinanceHttpClient(config);
  return {
    toolClient: {
      getJson: (path3, requestOptions) => httpClient.getJson(path3, requestOptions),
      postJson: (path3, body) => httpClient.postJson(path3, body),
      postFormData: (path3, formData) => httpClient.postFormData(path3, formData)
    },
    connect: async () => {
    },
    disconnect: () => {
...
var createCdpToolClient = (config) => {
  const browserClient = new BrowserClient(config);
  return {
    toolClient: {
      getJson: (path3, requestOptions) => browserClient.getJson(path3, requestOptions),
      postJson: (path3, body) => browserClient.postJson(path3, body),
      postFormData: (path3, formData) => browserClient.postFormData(path3, formData)
    },
    connect: () => browserClient.connect(),
    disconnect: () => browserClient.disconnect()
  };
};
var createSkill = (options = {}) => {
  const config = options.config ?? loadConfig(options.loadConfigOptions);
  const { toolClient, connect, disconnect } = config.cdpUrl ? createCdpToolClient(config) : createHttpToolClient(config);
  return {
    connect,
    disconnect,
    validate_session: () => validateSession({
      config,
      client: {
        getJson: (path3) => toolClient.getJson(path3)
      }
    }),
    publish_post: (input) => publishPost(input, {
      config,
      client: toolClient
    }),
    get_post_status: (input) => getPostStatus(input, {
      config,
      client: {
...
var runCli = async () => {
  ...
  try {
    ...
    if (command === "publish_post") {
      writeJson(await skill.publish_post(parsePayload(rawPayload)));
      return;
    }
    if (command === "get_post_status") {
      writeJson(await skill.get_post_status(parsePayload(rawPayload)));
      return;
    }
