...
class TestUploadCommand(unittest.TestCase):
    ...
    def test_upload_single_file(self, mock_get_client):
        ...
        try:
            ...
            # Verify API was called
            mock_client.upload_new_code_snips.assert_called_once()
            call_kwargs = mock_client.upload_new_code_snips.call_args.kwargs
            self.assertEqual(call_kwargs['task'], "test task")
            self.assertEqual(call_kwargs['succeeded'], True)
            self.assertEqual(call_kwargs['auto_vote'], True)
            self.assertEqual(len(call_kwargs['files_written']), 1)
    ...
    def test_upload_multiple_files(self, mock_get_client):
        ...
        try:
            ...
            call_kwargs = mock_client.upload_new_code_snips.call_args.kwargs
            self.assertEqual(len(call_kwargs['files_written']), 3)
    ...
    def test_upload_no_auto_vote(self, mock_get_client):
        ...
        try:
            ...
            call_kwargs = mock_client.upload_new_code_snips.call_args.kwargs
            self.assertEqual(call_kwargs['auto_vote'], False)
        finally:
            os.unlink(temp_path)
