...
class SkillScanner:
    ...
    def _scan_content(self, file_path: Path, lines: List[str]):
        ...
        for pattern_def in THREAT_PATTERNS:
            ...
            for i, line in self._iter_scan_lines(file_path, lines):
                stripped = line.strip()
                if not stripped:
                    continue
                if self._is_definition_noise(file_path, stripped):
                    continue
    ...
    def _should_skip_path(self, file_path: Path) -> bool:
        try:
            rel_parts = file_path.relative_to(self.skill_path).parts[:-1]
        except ValueError:
            rel_parts = file_path.parts[:-1]
        return any(part in IGNORED_DIR_NAMES for part in rel_parts)

    def _iter_scan_lines(self, file_path: Path, lines: List[str]) -> Iterator[Tuple[int, str]]:
        suffix = file_path.suffix.lower()
        if suffix not in {'.md', '.markdown', '.rst'}:
            for i, line in enumerate(lines, 1):
                yield i, line
            return

        # Markdown/RST text is noisy; scan only fenced code blocks.
        in_code_block = False
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
    ...
    def _is_definition_noise(self, file_path: Path, stripped_line: str) -> bool:
        if file_path.name == 'skill_scanner.py' and re.search(r'["\']pattern["\']\s*:', stripped_line):
            return True
