...
# Import the scanner
try:
    from skill_scanner import SkillScanner, ScanReport, format_markdown
except ImportError:
...
def _normalize_results(results):
    """Return (dict_view, scan_report_or_none) for robust UI rendering."""
    if isinstance(results, ScanReport):
        return asdict(results), results
    if is_dataclass(results):
        return asdict(results), None
...
def display_results(results, output_format: str, show_info: bool):
    """Display scan results in a user-friendly format."""
    results_dict, scan_report = _normalize_results(results)
    ...
    for finding in results_dict.get('findings', []):
    ...
    # Verdict
    verdict = results_dict.get('verdict', 'UNKNOWN')
    ...
    # Files scanned
    files_scanned = results_dict.get('files_scanned', [])
    if files_scanned:
        ...
        for i, finding in enumerate(findings):
            ...
            with st.container():
                ...
                <div class="finding-card {severity}">
                    ...
                    <strong>{finding.get('pattern_name', finding.get('category', 'Unknown'))}</strong><br/>
                    <em>{finding.get('file_path', finding.get('file', 'Unknown file'))}</em> 
                    {f"(Line {finding.get('line_number', finding.get('line', '?'))})" if finding.get('line_number', finding.get('line')) else ''}<br/>
                    {finding.get('description', '')}
                </div>
                """, unsafe_allow_html=True)
                
                matched_line = finding.get('line_content', finding.get('match'))
                if matched_line:
                    with st.expander("View matched code"):
                        st.code(matched_line, language='python')
    else:
    ...
    with col1:
        if scan_report is not None:
            md_report = format_markdown(scan_report)
            st.download_button(
            ...
            st.caption("Markdown export unavailable for this result format.")
    
    with col2:
        json_report = json.dumps(results_dict, indent=2)
        st.download_button(
