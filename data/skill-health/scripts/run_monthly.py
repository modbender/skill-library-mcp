#!/usr/bin/env python3
\"\"\"Run monthly analysis and print JSON to stdout (and optional file).\"\"\"
...
from skill_health.analysis.monthly import build_monthly_report, infer_month_end_date_from_bundle
from skill_health.load import load_health_data_from_directory, load_health_data_from_path


def main() -> int:
    parser = argparse.ArgumentParser(description=\"Monthly analysis (calendar month).\")
    ...
    parser.add_argument(\"--date\", type=str, default=None, help=\"Month end date (YYYY-MM-DD).\")
    ...
    month_end_date = (
        ...
        else infer_month_end_date_from_bundle(bundle)
    )
    payload = build_monthly_report(bundle, month_end_date=month_end_date)
    json_str = json.dumps(payload, indent=2, ensure_ascii=False)

    if args.output_dir is not None:
        args.output_dir.mkdir(parents=True, exist_ok=True)
        filename = f\"monthly_{month_end_date.isoformat()}\.json\"
