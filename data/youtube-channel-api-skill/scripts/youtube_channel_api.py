...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python youtube_channel_api.py <keywords> [upload_date]", flush=True)
        sys.exit(1)
    ...
    keywords = sys.argv[1]
    upload_date = sys.argv[2] if len(sys.argv) > 2 else "Any time"
    
    result = run_youtube_channel_task(api_key, keywords, upload_date)
    if result:
