...
export const DEFAULT_CONFIG: EnhancedLoopConfig = {
  enabled: false,
  planning: {
    enabled: true,
...
export class EnhancedAgentLoop {
  ...
  private state: EnhancedLoopState;
  private stateMachine: AgentStateMachine;
  private errorStore: ErrorLearningStore;
  private callbacks: EnhancedLoopCallbacks;

  constructor(
    config: Partial<EnhancedLoopConfig> = {},
    callbacks: EnhancedLoopCallbacks = {}
  ) {
  ...
  async run(
    ...
    try {
      ...
      if (shouldGeneratePlan(userGoal, this.state.currentPlan, this.config.planning)) {
        this.state.currentPlan = await generatePlan(
          userGoal,
          context,
          this.config.planning,
          llmCall
        );
        planGenerated = true;
      ...
      while (this.state.iterationCount < maxIterations) {
        ...
        if (this.config.planning.reflectionAfterTools && this.state.currentPlan) {
          ...
          const mockToolCall: ToolCall = { id: "mock", name: "mock", arguments: {} };
          const mockResult: ToolResult = { id: "mock", success: true };
          ...
          if (reflection.decision === "replan") {
            ...
            this.state.currentPlan = await replan(
              this.state.currentPlan,
              reflection.reason ?? "Reflection indicated replanning needed",
              this.config.planning,
              llmCall
            );
            replans++;
            ...
            if (this.callbacks.onHumanInputRequired) {
              ...
              await this.callbacks.onHumanInputRequired(
                reflection.reason ?? "Human input needed"
              );
        ...
        if (nextStep && this.state.currentPlan) {
          this.state.currentPlan = completeStep(
            this.state.currentPlan,
            nextStep.id,
            "Completed"
          );
  ...
  async executeTools(
    ...
    for (const tool of toolCalls) {
      ...
      if (this.config.execution.confidenceGates) {
        const { assessment, decision } = await gateAction(
          tool,
          context,
        ...
        if (decision.waitForHuman) {
          if (this.callbacks.onHumanInputRequired) {
            const response = await this.callbacks.onHumanInputRequired(
              decision.question ?? "Should I proceed?",
              decision.options
            );
      ...
      if (this.config.errorRecovery.enabled) {
        const { result, diagnosis, attempts } = await executeWithRecovery(
          tool,
          executor,
        ...
        if (
          this.config.errorRecovery.learnFromErrors &&
          diagnosis &&
          result.success &&
          attempts > 1
        ) {
  ...
  async executeToolsParallel(
    toolCalls: ToolCall[],
    executor: ToolExecutor
  ): Promise<{ results: Map<string, ToolResult>; metrics?: unknown }> {
 ...
 */
export function createEnhancedLoop(
  config?: Partial<EnhancedLoopConfig>,
  callbacks?: EnhancedLoopCallbacks
): EnhancedAgentLoop {
