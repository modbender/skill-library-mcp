"""Generate natural thinking/filler sounds using ElevenLabs TTS.

Creates short audio clips (um, hmm, let me see, etc.) in the configured voice
and saves them as WAV files in assets/thinking_sounds/.
...
def generate():
    ...
    out_dir = Path(__file__).resolve().parent / "assets" / "thinking_sounds"
    ...
    for i, phrase in enumerate(PHRASES):
        ...
        try:
            ...
            path = _mp3_to_wav(b"".join(stream), out_dir / f"thinking_{i:02d}.wav")
            print(f"    -> {path or 'FAILED'}")
        except Exception as e:
            print(f"    -> ERROR: {e}")

    print(f"\nDone! {len(list(out_dir.glob('*.wav')))} thinking sounds")
