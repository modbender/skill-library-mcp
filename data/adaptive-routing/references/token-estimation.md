...
Over a month of 2,000 daily requests that stay local (no escalation):
...
When a request escalates (local validation fails → cloud re-run), both the
local model tokens and cloud tokens are consumed. `track_savings.py summary`
reports `escalation_rate` so you can tune `complexity_threshold` to reduce it.
