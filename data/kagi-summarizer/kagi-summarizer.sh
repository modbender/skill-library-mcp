...
if [[ "$needs_build" -eq 1 ]]; then
  ...
  if command -v go >/dev/null 2>&1; then
    ...
    if [[ -n "$GO_VERSION" ]]; then
      ...
      if [[ "$GO_MAJOR" -lt 1 || ( "$GO_MAJOR" -eq 1 && "$GO_MINOR" -lt 26 ) ]]; then
        ...
        echo "Building kagi-summarizer from source..." >&2
  ...
  if [[ ! -x "$BIN" ]]; then
    ...
    echo "kagi-summarizer binary not found. Download pre-built binary from GitHub releases?" >&2
    echo "  $URL" >&2
