---
name: feishu-app-setup — 飞书开放平台应用自动化配置
description: 在飞书开放平台上配置一个 OpenClaw bot
  应用需要大量重复的点击操作。这个技能把整个流程自动化了：创建应用、添加机器人能力、批量导入权限、配置事件订阅、修改应用名称、发布版本——全部通过
  agent-browser 在浏览器中自动完成。
---
